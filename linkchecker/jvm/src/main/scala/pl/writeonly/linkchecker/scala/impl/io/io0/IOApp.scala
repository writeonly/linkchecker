...
import pl.writeonly.linkchecker.scala.common.states.api.io.ParallelStateIO
...
object IOApp extends SafeApp with ValidationAPIStateEffect[ParallelStateIO] {
  ...
  def effect(): ParallelStateIO = IOState.fromDomain(new Domain(domain)) |> IOState.run
}
